    ...
    <div class="content">
      ...
      <!-- KOLOM  ------>
          ...
          <div class="card">
              ...
              <input type="hidden" name="id_lain" value="<?=$id_lain;?>">
                ...
                <div class="float-right">
                  <a id="batal" href="<?=base_url('sjrekanan')?>"><div class='btn btn-danger'>Kembali</div>&nbsp;</a> 
          ...
          <div class="tombol_order card-footer">
                <div class="float-right">
                <button  id="finish_edit" onclick="finish_edit(<?=$id_lain;?>)" class="btn btn-info">Simpan Surat Jalan</button>
                </div>
                </div>
          </div>
          </div>
            ...
            <form class="form-horizontal" id="form-edit">
              ...
              <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                ...
                <div class="form-group row">
                    <label for="nama_barang" class="col-sm-4 col-form-label">Nama Barang</label>
                    <div class="col-sm-8">
                      <input  id="nama_barang_edit" onkeyup="return this.value=this.value.toUpperCase()" name="nama_barang" type="text" class="form-control form-control-sm" placeholder="" autocomplete="off" required>
                    </div>
                </div> 
                  <!-- MESIN -->
                <div class="form-group row">
                ...
                <div class="form-group row">
                 ...
                 <input type="hidden" id="id_lain_detail" name="id_lain_detail">
...
$.ajax({
     type: "POST",
     url: "<?=base_url('sjrekanan/edit_sjrekanan/').$id_lain;?>",
     data: csrf,
     success: function (response) {
      ...
      $.each(hasil, function (i, item) { 
        no++;
         $("#item_edit").append('<tr><td>'+no+'</td><td>'+hasil[i].nama_barang+'</td><td>'+hasil[i].nama_mesin+'</td><td>'+hasil[i].note+'</td><td>'+hasil[i].qty_kirim+' '+hasil[i].kode_satuan+'</td><td><button onclick="edit_detail('+hasil[i].id_lain+','+hasil[i].id_lain_detail+');" class="btn btn-xs btn-info"><i class="fa fa-edit"></i></button></td><td><button onclick="hapus_detail('+hasil[i].id_lain+','+hasil[i].id_lain_detail+');"  class="hapus btn btn-xs btn-danger"><i class="fa fa-trash"></i></button></td></tr>');
         $("#id_relasi").val(hasil[i].id_relasi).change();
      });
     }
   });
...
$(document).ready(function () {
 ...
 $("#form-order").submit(function (e) { 
   ...
   $.ajax({
     type: "POST",
     url: "<?=base_url('sjrekanan/create_detail')?>",
     data: $(this).serialize(),
     success: function (response) {
       $("#item_edit").empty();
       $("#nama_barang").val('');
...
function hapus_detail(id_lain,id_lain_detail) {
  if (!confirm('Yakin hapus order ini?')) return false;
  $.ajax({
    type: "POST",
    url: "<?=base_url('sjrekanan/del_detail/')?>"+id_lain+"/"+id_lain_detail,
    data: csrf,
    success: function (response) {
...
function edit_detail(id_lain,id_lain_detail) {
 $("#modal-edit").modal('show');  
 $.ajax({
   type: "POST",
   url: "<?=base_url('sjrekanan/edit_detail/')?>"+id_lain_detail,
   data: csrf,
   success: function (response) {
    var hasil = JSON.parse(response);
      var no=0;
      $.each(hasil, function (i, item) { 
       ...
       $("#id_lain").val(id_lain);
       $("#id_lain_detail").val(id_lain_detail);
      });
   }
 });
}
...
$("#form-edit").submit(function (e) { 
   ...
   $.ajax({
     type: "POST",
     url: "<?=base_url('sjrekanan/update_detail')?>",
     data: $(this).serialize(),
     success: function (response) {
       console.log(response);
       $("#item_edit").empty();
 ...
 function batal_all(id_lain){
  if (!confirm('Yakin membatalkan seluruh order lain ini?')) return false;
   $.ajax({
     type: "POST",
     ...
     success: function()
     {
      window.location.href='<?=base_url("lain");?>';
     }
   });
 }
 function finish_edit(id_lain) {
   var id_relasi = $("#id_relasi").val();
   $.ajax({
     type: "POST",
     url: "<?=base_url('sjrekanan/finish_edit/')?>"+id_lain+'/'+id_relasi,
     data: csrf,
     success: function (response) {
