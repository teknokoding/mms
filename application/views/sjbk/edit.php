    ...
    <div class="content">
      ...
      <!-- KOLOM  ------>
          ...
          <div class="card">
              ...
              <input type="hidden" name="id_bengkel" value="<?=$id_bengkel;?>">
                ...
                <div class="float-right">
                  <a id="batal" href="<?=base_url('sjbk')?>"><div class='btn btn-danger'>Kembali</div>&nbsp;</a> 
          ...
          <div class="tombol_order card-footer">
                <div class="float-right">
                  <a href="<?=base_url('sjbk/finish_edit/').$id_bengkel;?>"><?php $this->session->userdata('id_level')>6?$tombol="":$tombol='<button  class="btn btn-info">Simpan Surat Jalan</button>';echo $tombol; ?></a>
                </div>
                </div>
          </div>
          </div>
            ...
            <form class="form-horizontal" id="form-edit">
              ...
              <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                ...
                <div class="form-group row">
                 ...
                 <input type="hidden" id="id_bengkel_detail" name="id_bengkel_detail">
...
$.ajax({
     type: "POST",
     url: "<?=base_url('sjbk/edit_sjbk/').$id_bengkel;?>",
     data: csrf,
     success: function (response) {
      ...
      $.each(hasil, function (i, item) { 
        no++;
         $("#item_edit").append('<tr><td>'+no+'</td><td>'+hasil[i].nama_barang+'</td><td>'+hasil[i].nama_mesin+'</td><td>'+hasil[i].note+'</td><td>'+hasil[i].qty_kirim+' '+hasil[i].kode_satuan+'</td><td><button onclick="edit_detail('+hasil[i].id_bengkel+','+hasil[i].id_bengkel_detail+');" class="btn btn-xs btn-info"><i class="fa fa-edit"></i></button></td><td><button onclick="hapus_detail('+hasil[i].id_bengkel+','+hasil[i].id_bengkel_detail+');"  class="hapus btn btn-xs btn-danger"><i class="fa fa-trash"></i></button></td></tr>');
      });
     }
   });
...
$(document).ready(function () {
 ...
 $("#form-order").submit(function (e) { 
   ...
   $.ajax({
     type: "POST",
     url: "<?=base_url('sjbk/create_detail')?>",
     data: $(this).serialize(),
     success: function (response) {
       $("#item_edit").empty();
       $("#nama_barang").val('');
...
function hapus_detail(id_bengkel,id_bengkel_detail) {
  if (!confirm('Yakin hapus order ini?')) return false;
  $.ajax({
    type: "POST",
    url: "<?=base_url('sjbk/del_detail/')?>"+id_bengkel+"/"+id_bengkel_detail,
    data: csrf,
    success: function (response) {
...
function edit_detail(id_bengkel,id_bengkel_detail) {
 $("#modal-edit").modal('show');  
 $.ajax({
   type: "POST",
   url: "<?=base_url('sjbk/edit_detail/')?>"+id_bengkel_detail,
   data: csrf,
   success: function (response) {
    var hasil = JSON.parse(response);
      var no=0;
      $.each(hasil, function (i, item) { 
       ...
       $("#id_bengkel_detail").val(id_bengkel_detail);
      });
   }
 });
}
...
$("#form-edit").submit(function (e) { 
   ...
   $.ajax({
     type: "POST",
     url: "<?=base_url('sjbk/update_detail')?>",
     data: $(this).serialize(),
     success: function (response) {
       console.log(response);
       $("#item_edit").empty();
 ...
 function batal_all(id_bengkel){
  if (!confirm('Yakin membatalkan seluruh order bengkel ini?')) return false;
   $.ajax({
     type: "POST",
     url: "<?=base_url('sjbk/batal_all/')?>"+id_bengkel,
     data: csrf,
     success: function()
     {
      window.location.href='<?=base_url("bengkel");?>';
     }
   });
 }
</script>
