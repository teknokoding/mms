...
<div class="content-wrapper">
   ...
   <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">TAG TPM</h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-tool">
                    <select name="id_mesin" id="setid_mesin" required class="select2 form-control form-control-sm" placeholder="">
...
$.ajax({ 
   type: "POST",
   url: '<?=base_url('tpm/set_mesin/');?>'+id_mesin,
   data: csrf,
   cache: false,
...
$('.btn-riwayat').click(function()
{
  ...
  $("#riwayat").load('<?=base_url('tpm/riwayat/');?>'+id_tag); 
})
// RELEASE
$('.btn-release').click(function()
{
  var id_tag = $(this).attr('id_tag');
  $("#release").load('<?=base_url('tpm/release/');?>'+id_tag); 
})
document.title = "Tag TPM <?php if($this->session->userdata('id_mesin')=="ALL"){$mesinnya="SEMUA MESIN";}else{$mesinnya=$item->nama_mesin;} echo $mesinnya;?>";
</script>