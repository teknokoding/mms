<?php
foreach ($sjreform as $itemsingle) {
}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  <meta charset="utf-8">
  <title>Surat Jalan SJ-RFM-<?php echo date('my')."-".$itemsingle->id_reform;?></title>
...
<tbody>
		<tr>
			...
			<b>SURAT JALAN<br>
				No. SJ/RFM/<?php echo date('my')."-".$itemsingle->id_reform;?></b>
			</td>
		</tr>
		<tr>
	...
	<table class="table">
		...
		<tr>
			...
			<tr>
			<td>
				<b><?php $itemsingle->pic!=''?$pic=$itemsingle->pic:$pic='';
				$itemsingle->jabatan_pic!=''?$jabatan=" - ".$itemsingle->jabatan_pic:$jabatan='';
				echo $pic.$jabatan;?>
				<?php if($itemsingle->pic!='' || $itemsingle->jabatan_pic!=''){echo "<br>";}?>
				<?=$itemsingle->nama_klien;?><br></b>
				<?=$itemsingle->alamat_klien;?><br><br>
				Mohon diterima pisau sebagai berikut:
			</td>
			<td></td>
...
<br>
  ...
  <?php
  $no=0;
  foreach ($sjreform as $item) {
    $no++;
    echo"
    <tr>
    <td>$no</td>
		<td>$item->nama_pisau</td>
...
var opt = {
  filename:     'SJ-RFM-<?=$item->id_reform;?>',
  html2canvas:  { scale: 3 }
};
