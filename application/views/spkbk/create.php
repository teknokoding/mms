<?php
$id_spkbk = random_string('numeric',8);
?>
      ...
      <!-- KOLOM  ------>
          ...
          <div class="card">
              <div class="card-header bg-info">
                <h3 class="card-title">Form Order Bengkel</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              ...
              <input type="hidden" name="id_spkbk" value="<?=$id_spkbk;?>">
                ...
                <div class="float-right">
                  <a id="batal" href="<?=base_url('spkbk')?>"><div class='btn btn-danger'>Batal</div>&nbsp;</a> 
          ...
          <div class="tombol_order card-footer">
                <div class="float-right">
                  <button onclick="batal_all(<?=$id_spkbk;?>);" class='btn btn-danger'>Batal</button></td><td>
                  <a href="<?=base_url('spkbk/draft/').$id_spkbk;?>"><button  class="btn btn-warning">Simpan Draft</button></a>&nbsp;
                  <a href="<?=base_url('spkbk/finish/').$id_spkbk;?>"><?php $this->session->userdata('id_level')>5?$tombol="":$tombol='<button  class="btn btn-info">Simpan SPK</button>';echo $tombol; ?></a>
                </div>
                </div>
          </div>
          </div>
            ...
            <form class="form-horizontal" id="form-edit">
              ...
              <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                ...
                <div class="form-group row">
                 ...
                 <input type="hidden" id="id_spkbk_temp" name="id_spkbk_temp">
...
$(document).ready(function () {
 ...
 $("#form-order").submit(function (e) { 
   ...
   $.ajax({
     type: "POST",
     url: "<?=base_url('spkbk/create_temp')?>",
     data: $(this).serialize(),
     success: function (response) {
      ...
      $.each(hasil, function (i, item) { 
        no++;
         $("#item_temp").append('<tr><td>'+no+'</td><td>'+hasil[i].nama_barang+'</td><td>'+hasil[i].nama_mesin+'</td><td>'+hasil[i].spesifikasi+'</td><td>'+hasil[i].note+'</td><td>'+hasil[i].qty_minta+' '+hasil[i].kode_satuan+'</td><td>'+hasil[i].status_pesan+'</td><td><button onclick="edit_temp('+hasil[i].id_spkbk+','+hasil[i].id_spkbk_temp+');" class="btn btn-xs btn-info"><i class="fa fa-edit"></i></button></td><td><button onclick="hapus_temp('+hasil[i].id_spkbk+','+hasil[i].id_spkbk_temp+');"  class="hapus btn btn-xs btn-danger"><i class="fa fa-trash"></i></button></td></tr>');
      });
     }
   });
 });
...
function hapus_temp(id_spkbk,id_spkbk_temp) {
  if (!confirm('Yakin hapus order ini?')) return false;
  $.ajax({
    type: "POST",
    url: "<?=base_url('spkbk/del_temp/')?>"+id_spkbk+"/"+id_spkbk_temp,
    data: csrf,
    success: function (response) {
...
function edit_temp(id_spkbk,id_spkbk_temp) {
 $("#modal-edit").modal('show');  
 $.ajax({
   type: "POST",
   url: "<?=base_url('spkbk/edit/')?>"+id_spkbk_temp,
   data: csrf,
   success: function (response) {
    var hasil = JSON.parse(response);
      var no=0;
      $.each(hasil, function (i, item) { 
       ...
       $("#id_spkbk_temp").val(id_spkbk_temp);
      });
   }
 });
}
...
$("#form-edit").submit(function (e) { 
   ...
   $.ajax({
     type: "POST",
     url: "<?=base_url('spkbk/update_temp')?>",
     data: $(this).serialize(),
     success: function (response) {
       $("#item_temp").empty();
      var hasil = JSON.parse(response);
