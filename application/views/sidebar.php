 ...
 <aside class="main-sidebar sidebar-light-primary elevation-4">
    ...
    <div class="sidebar">
      ...
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        ...
        <div class="info">
          <a title="Profil User" href="<?=base_url('profil')?>" class="d-block judul"><?= $this->session->userdata('namalengkap');?></a>
        </div>
      </div>
      ...
      $menu = $this->db->get('sidebar')->result_array();
      $query = $this->db->query("SELECT parent FROM sidebar WHERE controller='$uri' LIMIT 1");
      $row = $query->row();
      ...
      foreach ($menu as $itemmenu) {
        ...
        if ($uri == $parent || $parent == $parentcontroller) {
          $aktif = 'active';
          $open = '';//kasih nilai menu-open buat otomatis open menu
        } else {
          $aktif = '';
          $open ='';
        } // UNTUK MENU PARENT
        $where = array('Parent' => $parent, 'Icon' => '' ,'Level LIKE' => '%' . $id_level . '%');
        $child = $this->db->get_where('sidebar', $where)->num_rows();
        if ($child == 0) {
          echo'
          <li class="nav-item">
          ...
          $wherechild = array('Parent' => $parent, 'Icon' => '', 'Level LIKE' => '%' . $id_level . '%');
            $this->db->order_by('Urutan', 'ASC');
            $submenu = $this->db->get_where('sidebar', $wherechild)->result_array();
            foreach ($submenu as $itemsubmenu) {
              if ($uri == $itemsubmenu['Controller']) {
                $aktifsub = ''; //untuk aktifkan sub menu
              } else {
                $aktifsub = '';
              }
              echo '
